...
while True:
    ...
    con = \
    cv2.findContours(bw_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0];
    try:
    ...
    try:
        if cv2.contourArea(my_con) > 90:
            ...
            cv2.line(roi, leftmost, (topmost[0], bottommost[1] - 80),
                     (0, 242, 225), 2);
            ...
            if angle < 15:
                mouse.click(
                    button='left');  # uncomment to activate the left click
                print('left clicked');
                pass
            else:
                pass
